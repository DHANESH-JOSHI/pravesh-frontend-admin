    ...
    switch (log.action) {
      case "status_update":
        if (log.oldValue && log.newValue) {
          return (
            <>
              {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="changed status from" /> {String(log.oldValue).replace(/_/g, " ")} to {String(log.newValue).replace(/_/g, " ")}
        ...
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="updated order status" />{orderRef && <> <OrderLink id={String(orderId)} /></>}
          </>
        );
      ...
      case "view":
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="viewed order" /> {orderRef ? <OrderLink id={String(orderId)} /> : ""}
          </>
        );
      ...
      case "view_list":
        ...
        if (query?.status) {
          return (
            <>
              {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="viewed orders filtered by" /> {String(query.status).replace(/_/g, " ")}
            </>
          );
        }
        if (query?.user) {
          return (
            <>
              {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="searched orders" /> for "{query.user}"
            </>
          );
        }
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="viewed all orders" />
          </>
        );
      ...
      case "items_updated":
        if (log.field) {
          return (
            <>
              {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="updated" /> {log.field}{orderRef && <> for <HighlightText text="order" /> <OrderLink id={String(orderId)} /></>}
            </>
          );
        }
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="updated order items" />{orderRef && <> <OrderLink id={String(orderId)} /></>}
          </>
        );
      ...
      case "feedback_updated":
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text="updated feedback" />{orderRef && <> for <HighlightText text="order" /> <OrderLink id={String(orderId)} /></>}
          </>
        );
      ...
      default:
        ...
        return (
          <>
            {adminId ? <UserLink id={adminId} name={adminName} /> : adminName} <HighlightText text={defaultText} />{orderRef && <> <OrderLink id={String(orderId)} /></>}
          </>
        );
    }
  };
              ...
              {logs.map((log) => {
                ...
                return (
                  <TooltipProvider key={log._id}>
                    <div className="group relative p-2 rounded-md border border-border/50 bg-card hover:bg-accent/30 hover:border-accent transition-all">
                      <div className="flex items-center gap-2">
